...
import { Send, Bot, User, Loader2, Sparkles, BookOpen, TrendingUp, AlertTriangle } from 'lucide-react';
...
const AiAgent: React.FC = () => {
  ...
  const handleSend = async (textInput: string) => {
    ...
    const userMsg: Message = { id: Date.now().toString(), role: 'user', text: textInput, timestamp: new Date() };
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: (Date.now() + 1).toString(),
        role: 'model',
        text: "I'm having trouble connecting to my financial data sources right now. Please try again in a moment.",
        timestamp: new Date()
      }]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="h-[calc(100vh-4rem-3rem)] md:h-[calc(100vh-4rem)] max-w-5xl mx-auto p-4 md:p-6 flex flex-col">
      <div className="flex-shrink-0 mb-6">
        ...
        <p className="text-gray-600 mt-2 ml-1">Your personal guide tailored to your {user.persona.toLowerCase().replace('_', ' ')} style.</p>
      </div>
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex gap-3 max-w-[85%] md:max-w-[75%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${msg.role === 'user' ? 'bg-gray-200' : 'bg-primary-100'}`}>
                  {msg.role === 'user' ? <User size={18} className="text-gray-600" /> : <Bot size={18} className="text-primary-600" />}
                </div>
                <div>
                   <div className={`p-4 rounded-2xl text-sm md:text-base prose prose-sm md:prose-base max-w-none ${
                    msg.role === 'user' 
                      ? 'bg-primary-600 text-white rounded-tr-none' 
                      : 'bg-gray-50 text-gray-800 rounded-tl-none border border-gray-100'
                    }`}>
                      <ReactMarkdown components={{
                        // Override some markdown styles for better chat look
                  ...
                  <span className={`text-xs text-gray-400 mt-1 block ${msg.role === 'user' ? 'text-right mr-1' : 'ml-1'}`}>
